            ...
            <>
                <FBox style={{ flex: 1, width: ScreenWidth - 40, alignItems: "center" }}>
                    <List.Item style={{ width: "100%" }} onPress={() => {
                        ...
                        description={`${medTime[item.takeMedicineTimeType].value} / ${item.dose}`} descriptionNumberOfLines={2}
                        descriptionStyle={{ color: colors.white }}
                        right={() => <FBox style={{ height: 60, alignItems: "center", justifyContent: "center" }}>
                            {!loading && <MaterialIcons name={"chevron-right"} size={24} color={colors.white} onPress={() => handleEditMed(item.medicineId)} />}
                        </FBox>} />
