...
export default function ConfirmClaimNFT(props: Props) {
    ...
    useEffect(() => {
        if (error) {
            if ('data' in error) {
                toastMessage({msg: (error.data as any)?.error as string});
    ...
    if (claimed)
        return (
            <MessageWithImage description={""} buttonMode={"text"}
                              imageUrl={require("../../../assets/icons/nft_bag.svg")} title={"Your got a NFT!"}
                              buttonText={"ホーム画面へ"}
            ...
            <FBox style={{...styles.card, borderColor: theme.colors.primary}}>
                <FBox style={{flex: 1}}>
                    <Text variant={"titleLarge"}
                          style={{...styles.itemText, fontWeight: '700', marginVertical: 50,}}>購入しますか?</Text>
                    ...
                    {props.nft.free &&
                        <Text variant='titleLarge' style={{...styles.itemText, marginTop: 10, fontWeight: '700'}}>You
                            can get it for FREE!</Text>}
                </FBox>
                {isLoading &&
